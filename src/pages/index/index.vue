...
<template>
  <view class="content">
    ...
    <view class="mt-[13.14758px] flex flex-col items-center space-y-[20rpx]">
      ...
      <view>
        <button class="w-64 text-white" :class="buttonClass" @click="increment">click here to inc {{ count }}</button>
      </view>

      <view class="test"> @apply 的用法，详见 `index.vue` 的 `style` 标签块 </view>
      ...
      <view>
        <button class="btn">此样式定义详见 `App.vue` 的 `@layer` 部分</button>
      </view>
    </view>
  </view>
</template>
...
.test {
  @apply flex h-[100px] w-[222.222px] items-center justify-center rounded-[40px] bg-[#123456] bg-opacity-[0.54] text-center text-[#ffffff];
}
